...
public:
     ...
     * @return operation result
     */
    ListOpResult get(size_t pos, T **value) const {
     ...
     * @return
     */
    ListOpResult nextIterator(size_t *pos) const {
        if (!this->addressValid(*pos) && *pos != 0)
            return LIST_OP_SEGFAULT;
     ...
     * @return
     */
    size_t nextIterator(size_t pos) const {
        if (!this->addressValid(pos) && pos != 0)
            return 0;
     ...
     * @return
     */
    size_t prevIterator(size_t pos) const {
        if (!this->addressValid(pos) && pos != 0)
            return 0;
     ...
     * @return
     */
    ListOpResult prevIterator(size_t *pos) const {
        if (!this->addressValid(*pos) && *pos != 0)
            return LIST_OP_SEGFAULT;
        *pos = this->storage[*pos].previous;
        return LIST_OP_OK;
